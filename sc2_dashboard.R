...
geojson <- fromJSON(file="geojson-counties-fips.json")
...
# load SC2 Data
sc2Data = read.csv(Sys.glob(file.path(getwd(), 'gisaid_*tsv')),sep="\t")
...
fig <- fig %>% colorbar(title = "Number of sequenced genomes")
fig <- fig %>% layout(
  title = 'SARS-CoV-2 Sequencing By County<br>(Hover for variant breakdown)',
  geo = g
)
